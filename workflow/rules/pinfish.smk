...
rule PinfishCollapseRawPartials:
    ...
    conda:
        "../envs/pinfish.yaml"
    shell:
       "collapse_partials -t {threads} -d {params.d} -e {params.e} -f {params.f} {input.cls_gff} > {output.cls_gff_col}"
...
rule PinfishCollapsePolishedPartials:
    ...
    conda:
        "../envs/pinfish.yaml"
    shell:
        "collapse_partials -t {threads} -d {params.d} -e {params.e} -f {params.f} {input.pol_gff} > {output.pol_gff_col}"
